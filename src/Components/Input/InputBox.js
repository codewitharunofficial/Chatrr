...
const InputBox = ({ reciever, convoId, sender }) => {
  ...
  const sendMessage = async () => {
    ...
    try {
      if (!!input) {
        socketServices.emit("send-message", qureiesForNewMessage);
        setInput("");
        return;
      }
  ...
  const reply = async () => {
    ...
    try {
      if (!!input) {
        socketServices.emit("reply-message", qureiesForReply);
        setInput("");
...

      {
        isReplying || repliedMessage && (
          <View style={{ backgroundColor: 'lightblue', width: '95%', height: 60, alignSelf: 'center', borderRadius: 10, paddingHorizontal: 10, paddingVertical: 4, borderWidth: StyleSheet.hairlineWidth, borderColor: 'gray' }} >
            <Entypo onPress={() => { setIsReplying(!isReplying); setRepliedMessage({}) }} name="circle-with-cross" size={20} color={'black'} style={{ position: 'absolute', right: -3, top: -6 }} />
            <Text style={{ fontSize: 18, color: 'black' }} >{selectedMessage?.message?.message}</Text>
          </View>
        )
      }
      ...
      <SafeAreaView edges={["bottom"]} style={styles.container}>
        ...
        >
          {input ? (
            <MaterialIcons
              style={styles.send}
              onPress={() => { isReplying || repliedMessage.message ? reply() : sendMessage() }}
              name="send"
              size={20}
              color={"white"}
            />
