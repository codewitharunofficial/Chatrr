...
const DeleteAccountScreen = () => {
    ...
    const deleteAccount = async () => {
        try {
            const {data} = await axios.post(`${process.env.EXPO_PUBLIC_BASE_URL}/api/v1/users/request-delete-account/${id}`, {otp: otp});
            if(data?.success){
  ...
  return (
    ...
    <View style={{width: '100%', height: '100%', flexDirection: 'row', flexWrap: 'wrap', paddingVertical: 20, gap: 10}} >
      ...
      <Text style={{fontSize: 14, color: 'red', marginHorizontal: 10}} >Please Understand that if youn delete this account:</Text>
      ...
      </View>
      {
        ...
        <Pressable onPress={handlePress} style={{backgroundColor: 'red', padding: 10, marginTop: 20, borderRadius: 20}} >
        ...
        <Pressable onPress={deleteAccount} style={{backgroundColor: 'red', padding: 10, marginTop: 20, borderRadius: 20}} >
