    ...
    <View style={{width: '100%', height: '100%'}} >
      {
        file && file?.type === 'image' ? (
            <IfImage uri={file.uri} type={file.type} name={file.name} sender={file.sender} reciever={file.reciever} />
            ) : (
            <IfAudio name={file?.name} uri={file?.uri} type={file?.type} sender={file.sender} reciever={file.reciever} />
        )
      }
    </View>
  )
